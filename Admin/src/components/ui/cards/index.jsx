...
export const CardFooter = ({ children, className, ...props }) => (
  <div
    className={twMerge("border-t pt-2 border-border", className)}
    {...props}
  >
    {children}
  </div>
...
export const CardImage = ({ src, alt, className = "", ...props }) => (
  <img
    src={src}
    alt={alt}
    className={twMerge("w-full h-auto rounded-t-lg  object-cover", className)}
    {...props}
  />
);
...
export const Card = ({ children, className, ...props }) => {
  return (
    <div
      className={twMerge(
        "text-sm bg-card flex flex-col gap-4 p-5 mt-1 ring-1 ring-border shadow-lg rounded-lg",
        className
      )}
      {...props}
    >
